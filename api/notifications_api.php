...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        AND start BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 7 DAY)
        AND status != 'completed'
    ");
    $stmt->execute([$user_id]);
    ...
    // 4. UPCOMING REMINDERS (Calendar reminders not sent yet)
    $stmt = $pdo->prepare("
        SELECT COUNT(DISTINCT cr.id) 
        FROM calendar_reminders cr
        INNER JOIN calendar_events ce ON cr.event_id = ce.id
        WHERE ce.user_id = ? 
        AND cr.sent = 0
        AND DATE_SUB(ce.start, INTERVAL cr.minutes_before MINUTE) <= NOW()
        AND ce.start > NOW()
    ");
    $stmt->execute([$user_id]);
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE user_id = ?
        AND DATE_ADD(contract_date, INTERVAL duration_months MONTH) BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)
    ");
    $stmt->execute([$user_id]);
    ...
    // 6. PENDING CAMPAIGN MESSAGES
    $stmt = $pdo->prepare("
        SELECT COUNT(*) FROM campains_queue 
        WHERE user_id = ? 
        AND status = 'QUEUE'
    ");
    $stmt->execute([$user_id]);
    ...
    // 7. DEADLINES (Next 3 days)
    $stmt = $pdo->prepare("
        SELECT id, title, start FROM calendar_events 
        WHERE user_id = ? 
        AND type = 'deadline'
        AND start BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 3 DAY)
        AND status != 'completed'
        ORDER BY start ASC
        LIMIT 3
    ");
    ...
    // Sort notifications by priority (0 = highest)
