...
function loadEvents(){
  $.getJSON('api.php?f=get_calendar_events&user_id=<?= $user_id ?>', function(resp){
    if(!resp.success){ toastr.error(resp.error||'Eroare încărcare'); return; }
    allEvents = (resp.data||[]);
...
function normalizeEvent(e){
  return {
    ...
    allDay: String(e.all_day) === '1',
    extendedProps: {
      ...e,
      all_day: String(e.all_day) === '1' ? 1 : 0,
      recurring: String(e.recurring) === '1' ? 1 : 0,
      recurrence: e.recurrence || '' // never null
    }
  };
}
...
function editEvent(ev){
  ...
  console.log(x);
  $('#event_id').val(ev.id);
  $('#event_type').val(x.type || 'todo');
  $('#event_priority').val(x.priority || 'medium');
  $('#event_title').val(ev.title || '');
  $('#event_description').val(x.description || '');
  $('#event_location').val(x.location || '');
  $('#event_attendees').val(x.attendees || '');
  $('#event_email_id').val(x.email_id || '');

  const start = ev.start ? moment(ev.start) : null;
  const end   = ev.end   ? moment(ev.end)   : null;

  $('#event_all_day').prop('checked', !!ev.allDay);
  if(ev.allDay){
    $('#event_start_date').val(start ? start.format('YYYY-MM-DD') : '');
    $('#event_start_time').val('');
    $('#event_end_date').val(end ? end.format('YYYY-MM-DD') : '');
    $('#event_end_time').val('');
    $('#event_start_time, #event_end_time').prop('disabled', true);
  }else{
    $('#event_start_date').val(start ? start.format('YYYY-MM-DD') : '');
    $('#event_start_time').val(start ? start.format('HH:mm') : '');
    $('#event_end_date').val(end ? end.format('YYYY-MM-DD') : '');
    $('#event_end_time').val(end ? end.format('HH:mm') : '');
    $('#event_start_time, #event_end_time').prop('disabled', false);
  }

  const isRecurring = String(x.recurring) === '1';
  $('#event_recurring').prop('checked', isRecurring);
  $('#recurringOptions').toggle(isRecurring);
  $('#event_recurrence').val(isRecurring ? (x.recurrence||'') : '');
