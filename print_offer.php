<?php
/*******************************************************
 * print_offer.php - WITH CONTACT SELECTOR ADDED
 * Minimal changes to original file - just added contact dropdown
 *******************************************************/
...
function loadTemplateData(PDO $pdo, ?int $templateId, int $userId): ?array {
    ...
    $stm = $pdo->prepare("SELECT data FROM offer_templates WHERE user_id = :uid ORDER BY updated_at DESC, id ASC LIMIT 1");
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .toolbar { max-width:900px; margin:12px auto 0; padding:0 24px 6px; display:flex; gap:8px; justify-content:space-between; align-items:center; flex-wrap:wrap; }
  ...
  <div class="meta">
    ...
    <div class="card">
      ...
      <?php if ($clientContact): ?>
        ...
        <?php if (!empty($clientContact['phone'])): ?>
          <div class="kv"><b>Telefon:</b> <?= h($clientContact['phone']); ?></div>
        <?php endif; ?>
        <?php if (!empty($clientContact['email'])): ?>
    ...
    <table>
      ...
      <?php
        ...
        foreach ($items as $it):
            ...
            $rowSubtotal = (float)($it['subtotal'] ?? ($qty * $unit));
      ?>
        <tr>
          <td><?= $idx++; ?></td>
